...
@DisplayName("TDD – Soma (add)")
class CalculatorAddExtendedTest {

    private final Calculator calc = new Calculator();

    @Nested
    @DisplayName("Faixa válida (positivos, negativos e zero)")
    class ValidRanges {
        ...
        void simple_pairs(double a, double b, double expected) {
            assertThat(calc.add(a, b), closeTo(expected, 1e-12));
        }
    ...
    @Nested
    @DisplayName("Decimais (comparação com tolerância)")
    class Decimals {
    ...
    class Extremes {
        ...
        void max_plus_one_is_still_max_due_to_rounding() {
            assertEquals(Double.MAX_VALUE, calc.add(Double.MAX_VALUE, 1.0), 0.0);
        }

        @Test
        @DisplayName("MAX_VALUE + MAX_VALUE → Infinity (comportamento atual)")
        void overflow_results_in_infinity_today() {
            assertTrue(Double.isInfinite(calc.add(Double.MAX_VALUE, Double.MAX_VALUE)));
        }

        @Test
        @DisplayName("(-MAX_VALUE) + (-MAX_VALUE) → -Infinity (comportamento atual)")
        void underflow_results_in_negative_infinity_today() {
            double r = calc.add(-Double.MAX_VALUE, -Double.MAX_VALUE);
    ...
    class InvalidInputs {
        ...
        @ParameterizedTest(name = "Infinity em add({0}, {1}) deve lançar IllegalArgumentException")
        @CsvSource(textBlock = """
            Infinity, 1
            -Infinity, 1
            1, Infinity
