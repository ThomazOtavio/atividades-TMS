...
public class Usuario {
    ...
    // Sobrecarga: permite informar o algoritmo JCA no momento de setar a senha
    public void setSenha(String senha, String algorithm) {
        validarNaoVazio(senha, "senha");
        PasswordHasher temp = new MessageDigestPasswordHasher(algorithm);
        this.senhaHash = temp.hash(senha);
    }

    public boolean autenticar(String senhaDigitada) throws ExceededAttemptsException {
        return autenticarInterno(senhaDigitada, this.hasher);
    }

    // Sobrecarga: autenticar informando o algoritmo JCA desejado
    public boolean autenticar(String senhaDigitada, String algorithm) throws ExceededAttemptsException {
        PasswordHasher temp = new MessageDigestPasswordHasher(algorithm);
        return autenticarInterno(senhaDigitada, temp);
    }

    private boolean autenticarInterno(String senhaDigitada, PasswordHasher hasherParaUsar) throws ExceededAttemptsException {
        ...
        String tentativaHash = hasherParaUsar.hash(senhaDigitada);
